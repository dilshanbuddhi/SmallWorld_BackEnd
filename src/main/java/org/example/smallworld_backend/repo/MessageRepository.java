...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT m FROM Message m WHERE " +
            "m.sender = :sender AND m.receiver = :receiver AND " +
            "m.read = false")
    List<Message> findUnreadMessagesBySenderAndReceiver(
    ...
    @Query("SELECT COUNT(m) FROM Message m WHERE m.receiver = :user AND m.read = false AND m.sender = :guid")
    int countUnreadMessages(@Param("user") User user, User guid);
    ...
    List<User> findReceivedUsers(User user);
}